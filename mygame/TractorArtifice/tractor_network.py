...
class AgentNet(nn.Module):#
    def __init__(self,in_fea=345):#345
        super().__init__()
        self.lstm1 = nn.LSTM(input_size=54, hidden_size=128, batch_first=True)
        self.lstm2 = nn.LSTM(input_size=54, hidden_size=128, batch_first=True)#本LSTM接受出牌的动作序列，可能只有1，也可能很多。
        ...
        self.mlp_act2 = nn.Sequential(  # 求动作平分,是actor网络
            nn.Linear(512+54, 512),#512是base网络的输出，128是单张牌之间lstm的输出
            nn.ReLU(),
            nn.Linear(512, 1),
        )
