...
from doudizhu_network_transofmer import AgentNet, AgentCriNet
# import pyro
...
class DppoWorkers:
    ...
    def initNet(self,path=""):#如果path存在,就是读取
        ...
        self.lr_c_now,self.lr_a_now=[self.args.value_lr,self.args.value_lr],[self.args.policy_lr,self.args.policy_lr]
        ...
        for i in range(3):
            ...
            self.optimizer.append([torch.optim.Adam(self.actor_net[i].parameters(), lr=self.args.policy_lr),torch.optim.Adam(self.actor_net[i].parameters(), lr=self.args.policy_lr)])
    ...
    def setNet(self,pid=0):#把网络给对应智能体
        ...
        self.agents[(pid+1)%3].enemyCriNet=self.critic_net_landlord
        self.agents[(pid) % 3].enemyCriNet = self.critic_net_peasant
        self.agents[(pid + 1) % 3].enemyCriNet = self.critic_net_landlord
        self.agents[(pid + 2) % 3].enemyCriNet = self.critic_net_landlord
    ...
    def lr_decay(self):
        if self.istrain[0]==1:
            self.lr_a_now[0]=max(0.000001,self.lr_a_now[0] -0.000001)
            self.lr_c_now[0]=max(0.000001, self.lr_c_now[0] - 0.000001)
            for p in self.optimizer[0][1].param_groups:
                p['lr'] = self.lr_a_now[0]
            for p in self.optimizer_critic_landlord.param_groups:
                p['lr'] = self.lr_c_now[0]
        if self.istrain[1] == 1:
            self.lr_a_now[1] = max(0.000001, self.lr_a_now[1] - 0.000001)
            self.lr_c_now[1] = max(0.000001, self.lr_c_now[1] - 0.000001)
            for i in range(1, 3):
                for p in self.optimizer[i][1].param_groups:
                    p['lr'] = self.lr_a_now[1]
            for p in self.optimizer_critic_peasant.param_groups:
                p['lr'] = self.lr_c_now[1]
    ...
    def playAGame(self,trainCnt,mini_epoch,trainDataset=True):#玩一局游戏，同时要收集数据
        ...
        deck=None
        if self.args.deckDataNpy_len!=0 or trainDataset==True:
            ...
            deck=self.playerDeck[self.istrain[1]==1].mkDeck()
    ...
    def train_agent(self,  readName=None):
        ...
        while True:
            ...
            for _ in range(epoch_size):
                ...
                leftCnt=[len(self.agents[i].player.cards) for i in range(3)]
                # print(leftCnt)
                leftCnt=leftCnt[0]-min(leftCnt[1],leftCnt[2])
                self.playerDeck[self.istrain[1] == 1].setReward((4-leftCnt)/4)
                # if (fen0-fen1)<0 and self.istrain[1] == 1 or (fen0-fen1)>0 and self.istrain[0] == 1:
                #     self.playerDeck[self.istrain[1] == 1].setReward(1)
                # else:
                #     self.playerDeck[self.istrain[1] == 1].setReward(-1)
                # self.lr_decay(self._trainCnt)
            ...
            self.playerDeck[self.istrain[1]==1].learn()
            ma5.add(avgAdp)
            y1List.append(rate)
