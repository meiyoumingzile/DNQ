...
class AgentCriNet(nn.Module):#
    def __init__(self,in_fea):#330
        super().__init__()
        self.tran =nn.Transformer(d_model=lstmInfea,nhead=3, num_encoder_layers=2, num_decoder_layers=2, dim_feedforward=lstmInfea,  batch_first=True, dropout=0.1)
        ...
        self.mlp_base = nn.Sequential(  # 求动作
            nn.LayerNorm(in_fea+lstmInfea*3),
            nn.Linear(in_fea+lstmInfea*3, mainfea),
            nn.Tanh(),
    ...
    def forward_base(self,baseFea,hisGameActi,hisActFea):  # 估计q，x是forward_base的结果
        x2= self.tran(hisActFea,hisActFea)
        # x2 = x2[:, -3:, :]
        x2 = torch.cat((x2[:, -3, :], x2[:, -2, :], x2[:, -1, :]), dim=1)
...
class AgentNet(nn.Module):#
    def __init__(self,in_fea):#330
        ...
        self.mlp_act2 = nn.Sequential(  # 求动作平分,是actor网络
            nn.LayerNorm(in_fea + lstmInfea*3+57+14),
            nn.Linear(in_fea + lstmInfea*3+57+14, mainfea),
            nn.Tanh(),
            nn.Linear(mainfea, mainfea),
            nn.LayerNorm(mainfea),
            nn.Tanh(),
    ...
    def forward_act(self, baseFea,hisActFea1,hisActFea2,actFea):  # actor网络，baseFea和hisActFea都是1*infea的大小
        ...
        # hisActFea = hisActFea2.repeat(batch_size, 1,1)
        # x1=x1.repeat(batch_size, 1)
        x2 = self.tran(hisActFea2,hisActFea2)
        # x2 = self.bn(x2)
        # print(x2.shape)
        x2 = torch.cat((x2[:, -3, :], x2[:, -2, :], x2[:, -1, :]), dim=1)
