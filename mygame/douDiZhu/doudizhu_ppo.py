...
class DppoWorkers:
    ...
    def getActionSpaceType(self,nowAgent):
        isDealer = (nowAgent.player.id != self.env.dealer)
        k = self.args.policykind[isDealer]
        return (k==-3 or k==-2 or k==3 or k==11 )
    ...
    def learnNet(self,playCnt):
        if self.istrain[0] == 1 and playCnt%self.args.learn_step==0:  # 训练地主
            self.agents[0].initLearn()
            self.agents[0].selflearn(self, updateNetwork)
            for j in range(3):
                self.agents[j].initMenory()
        if self.istrain[1] == 1 and playCnt%self.args.learn_step==0:  # 训练农民
            for i in range(1, 3):
                self.agents[i].initLearn()
                self.agents[i].selflearn(self, updateNetwork)
            for j in range(3):
                self.agents[j].initMenory()
    ...
    def train_agent(self,  readName=None):
        ...
        while True:
            ...
            self.winPlayerSum = [0, 0]  # 0是0和2赢，1是1和3赢。
            self.playerGradeSum = [0, 0]
            if len(ma5.que)==5 and (self.istrain[0]==1 and ma5.getAvg()>self.args.scthreshold or self.istrain[1]==1 and ma5.getAvg()<-self.args.scthreshold):#ma30大于0.6
                self.initactEpsl()
                ma5 =MACD(5)
                self.__playCnt=0
                pointList.append((self.__trainCnt,avgAdp))
