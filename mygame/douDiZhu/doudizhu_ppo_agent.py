...
class Dppoagent:
    ...
    def memoryTocpu(self,li):
        for teasor in li:
            teasor.cpu()
    def getReward(self):
        roundCnt = np.array([self.worker.agents[i].player.getMinStep() for i in range(3)])
    ...
    def chooseActFromNet_withDiv(self, roundId, env, maxAgent, preActQue,allActionList: list):  # ansUp代表较大的动作，ansDown代表较小的动作
        ...
        actid_2_prob = probList2[actid_2].item()
        act=allActionList[actid_2]
        self.roundBeginCnt = np.array([self.worker.agents[i].player.getMinStep() for i in range(3)])
        self.player.useAction(act, self.updateINF)
        reward=0
    ...
    def updateINF(self, cardi, appendix, allActList):
        ...
        if self.args.partialRewards != 0:
            reward = self.getReward() * self.args.partialRewards
        self.pushMemory(self.roundId,None,allActFea, baseFea,hisActFea, reward, None, (actid_2, actid_2_prob))
        self.roundBeginCnt = np.array([self.worker.agents[i].player.getMinStep() for i in range(3)])
        return actid_2
    ...
    def chooseActFromNet_test(self,roundId,env,maxAgent,preActQue,allActionList:list):
        baseFea = getBaseFea(env, self.player.id, preActQue, None, kind=self.args.seeType).cuda(
            self.worker.cudaID)  # self.id
        hisActFea = self.hisActTensor
        allActFea = getAllActionFea(self.env, allActionList).cuda(self.worker.cudaID)
        probList2 = self.net.forward_act(baseFea, self.hisGameActFea, hisActFea, allActFea).squeeze(dim=1)
        actid_2 = torch.argmax(probList2).item()
    ...
    def initLearn(self):#
        ...
        while memory_i<up:
            ...
            while(self.memoryIsDone[i]):#calc detla
                ...
                prev = v
                i+=1
                self.memoryTocpu([statei,hisActi,hisGameActi])
        ...
        # self.advantages = (self.advantages - mean) / (std+0.000001)
        self.advantages = self.advantages/10
        # print(self.advantages)
        # print()
        # print(self.advantages)
        # print(self.memoryReward)
    ...
    def selflearn_actor(self,updateNetwork):#updateNetwork为更新函数
        ...
        for _ in range(self.args.dataUseCnt):  # 训练critic
            ...
            for i in range(up):
                ...
                # print(probList2.shape)
                _,probList2=self.epslBoltzmann(probList2, None,0)#self.worker.actepsl
                ...
                log_nowRate=torch.clamp(torch.log(nowRate),-1000,1000)
                actor_loss = (-torch.max(torch.min(surr1, surr2),( 1 - self.CLIP_EPSL)*advantage)-self.args.entropy_coef*log_nowRate).mean()
                sumLoss_actor2.append(actor_loss)
                # print(loss_actor,actor_loss)
                # actor_loss.backward()
                # self.optimizer[2].step()
                self.memoryTocpu([allActFea,statei, hisActi,hisGameActi])
            updateNetwork(self,self.worker,sumLoss_actor1,sumLoss_actor2)
# x=torch.Tensor([0,0])
# x=torch.clamp(torch.log(x),-10,10)
# print(x)