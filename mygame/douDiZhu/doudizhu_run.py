...
parser.add_argument('--learn_noise', type=float, default=[0.05,0.05])
parser.add_argument('--learn_cstep', type=int, default=1000)
...
parser.add_argument('--istrain', type=list, default=[0,1,1])#1代表训练
parser.add_argument('--policykind', type=list, default=[1,1,1])
parser.add_argument('--shape', type=tuple, default=(1,350))#初始时训练那个人
parser.add_argument('--gama', type=float, default=1)
parser.add_argument('--lanta', type=float, default=0.98)
...
parser.add_argument('--pltDataLen', type=int, default=500)
parser.add_argument('--pltData_i', type=int, default=2)
parser.add_argument('--scthreshold', type=float, default=0.5)
parser.add_argument('--seeType', type=int, default=0)#1是不能看见
parser.add_argument('--partialRewards', type=float, default=0.05)#1是不能看见
parser.add_argument('--beginNet', type=str, default="2023-07-03-12-09")#1模型初始参数
args = parser.parse_args()
loadnpy(args,'eval_data.npy')#eval_data.npy
print("cuda:"+str(args.cudaIDList[0]))
worker=DppoWorkers(0,args)
worker.train_agent(args.beginNet)#2023-04-13-23-44，2023-04-13-18-47
