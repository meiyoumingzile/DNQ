...
from graphDou.graphDou_network import lstmInfea
from doudizhu_game import Doudizhu,Action,Player,dfsPrintActList
from graphDou.graphDou_encoder import handTo01code, getAllActionFea, getBaseFea, actTo01code, actZeros, getAllActionKindFea
import pyro
import pyro.distributions as dist

from doudizhu_game import _sortAction_cmp1, baselinePolicy_INFfun
...
class Dppoagent:
    ...
    def initHisGameActFea(self,li):
        n = len(li)
        if n == 0:
            self.hisGameActFea = torch.zeros((1, 1, lstmInfea)).cuda(self.worker.cudaID)
        else:
    ...
    def pushMemory(self,roundId,mask,allActList,baseFea,hisFea,reward,act1,act2):#allAct内部是多个list
        ...
        self.memoryHisGameFea.append(self.hisGameActFea)
    def getReward(self):
    ...
    def chooseAct_random2(self,roundId,env,maxAgent,preActQue,allActionList:list):  # 无脑出个数最多的
        n = len(allActionList)
        allActionList.sort(key=cmp_to_key(_sortAction_cmp1))
        newList = []
        prelen = -1
    ...
    def updateINF(self, cardi, appendix, allActList):
        baseFea,graphList = getBaseFea(self.env, self.player.id, self.preActQue,appendix,kind=self.args.seeType)  # self.id

        hisActFea = self.hisActTensor

        allActFea = getAllActionFea(self.env,allActList)
        probList2_0 = self.net.forward_act(baseFea,graphList,hisActFea, allActFea,self.worker.cudaID).squeeze(dim=1)
        ...
        self.pushMemory(self.roundId,None,allActFea, (baseFea,graphList),hisActFea, 0, None, (actid_2, actid_2_prob))
        return actid_2

    def chooseActFromNet_withDiv_test(self,roundId, env, maxAgent, preActQue,allActionList: list):
        baseFea,graphList = getBaseFea(env, self.player.id, preActQue,None,kind=self.args.seeType)  # self.id
        hisActFea = self.hisActTensor
        allActFea = getAllActionFea(self.env, allActionList).cuda(self.worker.cudaID)
        probList2 = self.net.forward_act(baseFea,graphList,hisActFea, allActFea,self.worker.cudaID).squeeze(dim=1)
    ...
    def chooseActFromNet_withDiv(self, roundId, env, maxAgent, preActQue,allActionList: list):  # ansUp代表较大的动作，ansDown代表较小的动作
        ...
        allActFea = getAllActionFea(self.env,allActionList)
        probList2_0 = self.net.forward_act(baseFea,graphList,hisActFea, allActFea,self.worker.cudaID).squeeze(dim=1)
        ...
        self.pushMemory(roundId, None, allActFea, (baseFea,graphList), hisActFea, reward, None, (actid_2, actid_2_prob))
        # act.println()
        return act
    ...
    def initLearn(self):#
        ...
        while memory_i<up:
            ...
            hisGameActi = self.memoryHisGameFea[memory_i]
            hisActi = self.memoryHisFea[memory_i]
            self.detlaList=[]
            prev = self.critic_net.forward(statei[0],statei[1],hisActi,self.worker.cudaID)
            i=memory_i+1
            ...
            while(self.memoryIsDone[i]):#calc detla
                ...
                v = self.critic_net.forward(statei[0],statei[1],hisActi,self.worker.cudaID)
    ...
    def selflearn_critic(self,l,r,updateNetwork):
        ...
        for _ in range(self.args.dataUseCnt):  # 训练critic
            ...
            for i in range(l,r):
                ...
                hisGameActi = self.memoryHisGameFea[i]
                v = self.critic_net.forward(statei[0],statei[1],hisActi,self.worker.cudaID)

                if done:
                    next_statei = self.memoryBaseFea[i + 1]
                    next_hisActi = self.memoryHisFea[i + 1]
                    next_v = self.critic_net.forward(next_statei[0],next_statei[1],next_hisActi,self.worker.cudaID)
    ...
    def selflearn_actor(self,updateNetwork):#updateNetwork为更新函数
        ...
        for _ in range(self.args.dataUseCnt):  # 训练critic
            ...
            for i in range(up):
                ...
                #下面是更新act网络
                probList2 = self.net.forward_act(statei[0],statei[1],hisActi,allActFea,self.worker.cudaID).squeeze(dim=1)
                # print(probList2.shape)
