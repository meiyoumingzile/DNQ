...
class MyPar():#充当静态类
    ...
    CLIP_EPSL = 0.1
    state_dim=490
    device=torch.device('cuda', args.cudaID)
    cudaID=args.cudaID
par=MyPar()
...
class Agent(object):#智能体，蒙特卡洛策略梯度算法
    ...
    def __init__(self,beginShape):
        ...
        # x = -torch.prod(torch.Tensor([1]).to(torch.device('cuda', 0))).item()
    ...
    def learn(self):#学习,训练ac算法2个网络
        # t = self.rangeList
        t=np.random.choice(self.MEMORY_CAPACITY, BATCH_SZ)
        # print(t.shape)
...
def train(beginShape, cartPole_util=None):
    ...
    for episode_i in range(EPOCHS_CNT):#循环若干次
        ...
        while (not is_terminal):
            ...
            agent.MEMORY_CAPACITY = min(MEMORY_CAPACITY,buffer.memory_i)
            if buffer.memory_i%5==0 and buffer.memory_i>BATCH_SZ:
                agent.learn()
        # if episode_i%10==0:
        #     # checkWD()
