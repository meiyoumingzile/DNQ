...
class CC():
    ...
    def __init__(self,randomOb=False):#randomOb代表随机生成墙
        ...
        self.ini_borad = np.zeros((self.SZ + 2, self.SZ + 2))  # 0和SZ+1代表墙
        self.board = np.zeros((self.SZ + 2, self.SZ + 2))  # 0代表空，1代表蛇身体，2代表头，3代表墙壁，-1代表食物
        self.head = None
        self.rear = None
        self.fp = [(0, 1), (0, -1), (1, 0), (-1, 0)]
        self.foodList = [(1, 1), (0, 0)]
    ...
    def reset(self):
        self.stepCntUp=self.SZ*self.SZ*1+1
        self.nowstepCnt=0
        self.stepCnt=0
        self.snakeLen=3
        self.emptyCnt=self.SZ * self.SZ - self.snakeLen
